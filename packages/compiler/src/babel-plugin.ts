// i never touched this code this was generated by claude given some specs :D
import type { PluginObj, PluginPass } from "@babel/core";
import * as t from "@babel/types";

const KAORI_PACKAGE_NAME = "kaori.js";

function findNonConflictingName(
	baseName: string,
		...
		visitor: {
			Program: {
				enter(path, state) {
					...
					const kaoriImports = new Map<string, string>(); // imported name -> local name
					...
					if (!state.hasHtmlImport || !state.hasComponentImport) {
						...
						if (!state.hasComponentImport) {
							state.componentFunctionName = findNonConflictingName(
								"component",
								existingNames
							);
						}
				...
				exit(path, state) {
					...
					if (state.needsComponentImport && !state.hasComponentImport) {
						imports.push(
							t.importSpecifier(
								t.identifier(state.componentFunctionName!),
					...
					if (state.needsHtmlImport && !state.hasHtmlImport) {
						imports.push(
							t.importSpecifier(
								t.identifier(state.htmlTagName!),
								t.identifier("html")
							)
						);
					}
					...
					if (imports.length > 0) {
						const kaoriImport = t.importDeclaration(
							imports,
...
function needsGetterWrapping(expression: t.Expression): boolean {
	...
	function traverse(node: t.Node) {
		if (hasDynamicAccess) return; // Early exit if already found
...
function createTemplateElement(
	...
	const templateElements = templateParts.map((part, index) =>
		t.templateElement(
...
type TemplateChildResult =
	| { type: "static"; content: string }
	| { type: "dynamic"; expression: t.Expression };

function processJSXChildrenForTemplate(
	children: (
		| t.JSXElement
		| t.JSXFragment
	...
	const results: TemplateChildResult[] = [];
	...
	for (const child of children) {
		if (t.isJSXText(child)) {
			...
			const fragmentResults = processJSXChildrenForTemplate(
				child.children,
				state
			);
